...
let ``can check if in range``() =
    ...
    (VersionRange.AtLeast "2.2").IsInRange "2.13" |> shouldEqual true
    (VersionRange.AtLeast "2.2").IsInRange "12.0" |> shouldEqual true
    ...
    (VersionRange.Between("2.2","3.0")).IsInRange "2.13" |> shouldEqual true
