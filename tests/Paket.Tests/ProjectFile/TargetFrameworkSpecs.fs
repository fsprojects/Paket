...
let ``should detect TargetFramework in Project2 proj file``() =
    ProjectFile.Load("./ProjectFile/TestData/Project2.fsprojtest").Value.GetTargetProfile().ToString()
    |> shouldEqual "net40"

[<Test>]
let ``should detect net40 in empty proj file``() =
    ProjectFile.Load("./ProjectFile/TestData/Empty.fsprojtest").Value.GetTargetProfile().ToString()
    |> shouldEqual "net40"

[<Test>]
let ``should detect silverlight framework in new silverlight project2``() =
    ProjectFile.Load("./ProjectFile/TestData/NewSilverlightClassLibrary.csprojtest").Value.GetTargetProfile()
