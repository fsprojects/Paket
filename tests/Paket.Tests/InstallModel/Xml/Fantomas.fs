...
let ``should generate full Xml for Fantomas 1.5``() = 
    ...
    let used = [NormalizedPackageName (PackageName "fantoMas"), { Name = PackageName "fantoMas" ; CopyLocal = true; ImportTargets = true; FrameworkRestrictions = []; OmitContent = false }] |> Map.ofSeq
...
let ``should not generate full Xml for Fantomas 1.5 if not referenced``() = 
    ...
    let project = ProjectFile.Load("./ProjectFile/TestData/Empty.fsprojtest").Value
    let completeModel = [NormalizedPackageName (PackageName "Fantomas"),model] |> Map.ofSeq
    let used = [NormalizedPackageName (PackageName "blub"), { Name = PackageName "blub"; CopyLocal = true; ImportTargets = true; FrameworkRestrictions = []; OmitContent = false } ] |> Map.ofSeq
    project.UpdateReferences(completeModel,used,false)
