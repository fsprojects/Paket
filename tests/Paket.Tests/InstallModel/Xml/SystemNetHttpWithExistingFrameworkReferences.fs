...
let ``should generate Xml for System.Net.Http 2.2.8``() = 
    ...
    let _,targetsNodes,chooseNode,_,_ = ProjectFile.TryLoad("./ProjectFile/TestData/FrameworkAssemblies.fsprojtest").Value.GenerateXml(model,Map.empty,true,true,None)
    chooseNode.OuterXml
    |> normalizeXml
