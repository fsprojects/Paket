...
let ``should resolve simple config1``() = 
    ...
    resolved.["Rx-Core"] |> shouldEqual "2.1"
    resolved.["Castle.Windsor-log4net"] |> shouldEqual "3.3"
    resolved.["Castle.Windsor"] |> shouldEqual "2.1"
    resolved.["log4net"] |> shouldEqual "1.1"
    resolved.["log"] |> shouldEqual "1.2"
