...
module Program =
    ...
    let processResults (results:ParseResults<Command>) =
    ...
    let main() =
        ...
        use _consoleTrace = Logging.event.Publish |> Observable.subscribe Logging.traceToConsole     
        try
            let results = paketParser.ParseCommandLine(raiseOnUsage = true)
...
type public Program =
    ...
    static member Exec (command:string) =
        let paketLog = StringBuilder()
        use _builderTrace = Logging.event.Publish |> Observable.subscribe (Logging.traceToBuilder paketLog)
        let results = paketParser.Parse [|command|]
        processResults results
        paketLog.ToString()
        
    
    static member Exec (command:string,workingDir:string) =
        let initialDir = System.IO.Directory.GetCurrentDirectory()
        System.IO.Directory.SetCurrentDirectory workingDir
        let result = Program.Exec command 
