...
let combineRestrictions x y =
    ...
    | FrameworkRestriction.Between(min1,max1) ->
        ...
        | FrameworkRestriction.Portable _ -> []
        | FrameworkRestriction.AtLeast r -> if min1 <= r && r <= max1 then [FrameworkRestriction.Between(r,max1)] else if r <= min1 && r <= max1 then [FrameworkRestriction.Between(min1,max1)] else []
