...
let internal findVersionsForPackage(v3Url, auth, package:Domain.PackageName, includingPrereleases, maxResults) =
    async {
        let! response = safeGetFromUrl(auth,sprintf "%s?id=%O&take=%d%s" v3Url package (max maxResults 100000) (if includingPrereleases then "&prerelease=true" else ""), acceptXml) // Nuget is showing old versions first
        match response with
        | Some text ->
            let versions =
