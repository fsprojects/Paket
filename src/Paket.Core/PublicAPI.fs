...
type Dependencies(dependenciesFileName: string) =
   ...
   /// Adds the given package with the given version to the dependencies file.
    ...
    /// Restores the given paket.references files.
    member this.Restore(force: bool, group: string option, files: string list, touchAffectedRefs: bool, ignoreChecks, failOnChecks, targetFramework, additionalReferences) : unit =
        ...
        RestoreProcess.Restore(dependenciesFileName,None,force,Option.map GroupName group,files,ignoreChecks, failOnChecks, targetFramework, additionalReferences)

    /// Restores the given paket.references files.
    member this.Restore(force: bool, group: string option, project: string, touchAffectedRefs: bool, ignoreChecks, failOnChecks, targetFramework, additionalReferences) : unit =
        if touchAffectedRefs then
            let packagesToTouch = RestoreProcess.FindPackagesNotExtractedYet(dependenciesFileName)
            this.Process (FindReferences.TouchReferencesOfPackages packagesToTouch)
        RestoreProcess.Restore(dependenciesFileName,Some project,force,Option.map GroupName group,[],ignoreChecks, failOnChecks, targetFramework, additionalReferences)

    /// Restores packages for all available paket.references files
    /// (or all packages if onlyReferenced is false)
    member this.Restore(force: bool, group: string option, onlyReferenced: bool, touchAffectedRefs: bool, ignoreChecks, failOnFailedChecks, targetFramework, additionalReferences): unit =
        if not onlyReferenced then 
            this.Restore(force,group,[],touchAffectedRefs,ignoreChecks,failOnFailedChecks,targetFramework, additionalReferences) 
        else
            let referencesFiles =
                ProjectFile.FindAllProjects this.RootPath
                |> Array.choose (fun p -> p.FindReferencesFile())
            if Array.isEmpty referencesFiles && additionalReferences = None then
                traceWarnfn "No paket.references files found for which packages could be installed."
            else
                this.Restore(force, group, Array.toList referencesFiles, touchAffectedRefs, ignoreChecks,  failOnFailedChecks, targetFramework, additionalReferences)
